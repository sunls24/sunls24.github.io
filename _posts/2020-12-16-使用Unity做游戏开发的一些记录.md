...
```
SuppressMessage特性只是为了隐藏Rider关于可能多次迭代的警告，事实上不会有这个问题。  
随机的数据类需继承IRandomItem接口，并实现Weight属性。  
...
```
可传入func指定分割后的数组类型，转换为int则需传入int.Parse。
...
```
当游戏数值非常大，超出decimal和double的精度时，需要使用BigInteger类型。  
进行乘除运算时使用double，加减以及存储时使用BigInteger。  
